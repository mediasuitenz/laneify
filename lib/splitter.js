...
module.exports = function (osmJsonPath) {
  ...
  var rawData
  try {
    // TODO: wtf is up with osmJsonPath not working, tried all sorts of combinations of __dirname
    // in here, __dirname in the caller (probably better, this module should receive a
    // fully qualified path to the json file)
    rawData = fs.readFileSync('osm.json')
  } catch (e) {
    console.log('Error reading data from', osmJsonPath, ':', e);
    return;
  }
  ...
  function setLaneData(lanes) {
    lanes.forEach(function (lane) {
      ...
      lane.properties.uid = rand()
      lane.properties.user = 'Media Suite'
    })
  }
  ...
  data.features = data.features.filter(function (feature) {
    return isRoad(feature)
  })

  var allLanes = []
  data.features.forEach(function (road) {
    var newLanes = []
    if (isOneWay(road)) {
      newLanes.push(road)
    } else {
      newLanes = createLanes(road)
    }

    allLanes = allLanes.concat(newLanes)
  })
  data.features = allLanes
