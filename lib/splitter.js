...
/**
 * Check if this is a road (by OSM's standard)
 * @param  {object} road  OSM GeoJSON LineString
 * @return {Boolean}
 */
function isRoad(feature) {
  var validRoadTypes = /motorway|trunk|primary|secondary|tertiary|unclassified|residential/
  var highway = feature.properties.highway

  return feature.geometry.type === 'LineString' && highway && validRoadTypes.test(highway)
}


module.exports = function splitter(road, options) {
  assert(!!road, '`road` must be supplied (GeoJSON object)')
  assert(isRoad(road), '`road` supplied must be a LineString with valid `highway` property')
